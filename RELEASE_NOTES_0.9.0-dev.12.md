# iDotMatrix WLED Usermod 0.9.0-dev.12
...
- Fixes the indeterminate Carousel activity bar on hardware by scheduling a periodic WLED refresh while the transfer indicator is visible. The animation clock is now independent of BLE chunk arrival and custom-effect redraw timing.
...
- When the quiet-period confirms that the transfer session has finished, the status bar fills completely before Carousel playback resumes.
